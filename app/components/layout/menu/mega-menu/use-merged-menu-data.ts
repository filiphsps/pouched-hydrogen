...
export function useMergedMenuData(): MergedMenuItem[] {
    ...
    return useMemo(() => {
        ...
        return headerMenu.items.map((nativeItem) => {
            ...
            const match = megaMenuItems.find((megaItem) => {
                ...
                const labelMatch =
                    getField(megaItem, "trigger_label")
                        ?.toLowerCase()
                        .trim() === nativeItem.title.toLowerCase().trim();
            ...
            const sections: MegaMenuSection[] = dropdownContent.map(
                (section) => {
                    ...
                    const hideResourceImages =
                        (getField(section, "hide_resource_images") ||
                            "false") === "true";

                    const links: MegaMenuLink[] = linksRef.map(
                        (menuLinkMetaobject) => {
                            const customTitle = getField(
                                menuLinkMetaobject,
                                "title",
                            );
                            ...
                            const pageRef = getReference(
                                menuLinkMetaobject,
                                "page",
                            );
                            const blogRef = getReference(
                                menuLinkMetaobject,
                                "blog",
                            );
                            const articleRef = getReference(
                                menuLinkMetaobject,
                                "article",
                            );
    ...
    }, [headerMenu, megaMenu, descriptionMode, descriptionEnding]);
}
